...
async def add_contract_handler(
    user: user_dependency, client_id: int, contract: Contract
):
    added_contract: Contract = contract.add(
...
async def update_contract_handler(
    user: user_dependency, contract_id: int, client_id: int, contract: Contract
):
    updated_contract = contract.update(
...
async def show_contract_handler(
    user: user_dependency, contract_id: int, client_id: int
):
    contract: Contract = show_contract(
        contract_id=contract_id, client_id=client_id, user_id=user.get("user_id")
    )
...
async def list_user_contracts_handler(user: user_dependency, client_id: int):
    contract_list = list_user_contracts(
        client_id=client_id, user_id=user.get("user_id")
    )
...
async def delete_contract_handler(
    user: user_dependency, contract_id: int, client_id: int
):
    if (
        delete_user_contract(
