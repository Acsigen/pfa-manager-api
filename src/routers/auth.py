...
async def auth_handler(form_data: Annotated[OAuth2PasswordRequestForm, Depends()]):
    authenticated_user_id: int = authenticate_user(username=form_data.username, password=form_data.password)
    if type(authenticated_user_id) is int:
        access_token = create_access_token(username=form_data.username, user_id=authenticated_user_id, expires_delta=timedelta(minutes=60))
        return {"access_token": access_token, "token_type": "bearer"}


async def get_current_user(token: Annotated[str, Depends(dependency=OAuth2PasswordBearer(tokenUrl="token"))]):
