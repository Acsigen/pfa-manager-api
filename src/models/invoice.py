...
class Invoice(BaseModel):
    ...
    def add(self):
        ...
        self.user_id = 0 
        data = (self.name, self.currency, self.exchange_rate, self.invoice_date, self.due_date, self.status, self.client_id, self.user_id)
        try: 
    ...
    def update(self, invoice_id):
        ...
        self.user_id = 0
        self.id = invoice_id
        data = (self.name, self.currency, self.exchange_rate, self.invoice_date, self.due_date, self.status, self.id)
        try: 
...
def show_invoice(invoice_id: int):
    ...
    try: 
        ...
        invoice: Invoice = Invoice(id=int(row[0]),
                            name=row[1],
                            currency=row[2],
                            exchange_rate=row[3],
                            invoice_date=row[4],
